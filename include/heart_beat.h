...
public:
   ...
   */
  static std::shared_ptr<HeartBeatTask> create(const std::string &name,
  ...
  {
    auto instance = std::shared_ptr<HeartBeatTask>(new HeartBeatTask(name, state_manager, heartbeat_interval_ms, response_timeout_ms));
...
protected:
  ...
  HeartBeatTask(const std::string &name = "HeartBeat",
                ...
                std::chrono::milliseconds heartbeat_interval_ms = std::chrono::milliseconds(10000),
                std::chrono::milliseconds response_timeout_ms = std::chrono::milliseconds(3000)) : task::Task(name),
  state_manager(state_manager), response_timeout_ms(response_timeout_ms)
  {
...
private:
  ...
  struct TaskStatus
  {
    ...
    std::chrono::steady_clock::time_point last_response_time; ///< Time the last response was received
  };

  std::map<std::shared_ptr<task::Task>, TaskStatus> monitored_tasks;  ///< Map of monitored tasks and their status
  ...
  task::TaskState unresponsive_action = task::TaskState::ERROR; ///< Action to take when a task is unresponsive
