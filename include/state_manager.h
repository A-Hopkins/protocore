...
const std::chrono::milliseconds STATE_TRANSITION_TIMEOUT =
    std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::seconds(5)); ///< Default timeout for state transitions.
 ...
 * between the tasks and the state manager. In the case a task cannot respond but has set its
 * internal state. States and tasks are tracked with a heap allocated unordered_map. Because it is
 * tracking states and pointers that should be allocated at initialization of the system.
 */
...
public:
   ...
   */
  void set_task_registration_observer(std::shared_ptr<task::Task> observer)
  {
    task_registration_observer = observer;
  }
   ...
   */
  bool demand_state_transition(task::TaskState           new_state,
                               std::chrono::milliseconds timeout = STATE_TRANSITION_TIMEOUT);
...
protected:
   ...
   * necessary initialization, such as subscribing to the appropriate message types. The default
   * implementation subscribes the StateManager to the StateAckMsg message type. Derived classes may
   * override this function to perform additional initialization tasks.
   */
  virtual void on_initialize() override { safe_subscribe(msg::Type::StateAckMsg); }

private:
  std::map<std::shared_ptr<task::Task>, task::TaskState>
                          task_states; ///< Tracks the current state of each registered task.
  ...
  std::condition_variable
                  state_transition_cv; ///< Condition variable used to coordinate state transitions.
  ...
  std::shared_ptr<task::Task> task_registration_observer; ///< Weak pointer to the HeartBeatTask
                                                          ///< that observes task registrations.
