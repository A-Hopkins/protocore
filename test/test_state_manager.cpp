...
public:
   ...
   */
  static std::shared_ptr<DelayedAckMockTask> create(const std::string &name, std::chrono::milliseconds delay)
  {
  ...
  void process_message(const msg::Msg &msg) override
  {
    ...
    if (get_current_state() == task::TaskState::NOT_STARTED || get_current_state() == task::TaskState::IDLE)
    {
...
TEST_F(StateManagerTest, DemandStateTransitionTimeout)
{
  ...
  bool success = sm->demand_state_transition(task::TaskState::RUNNING, std::chrono::milliseconds(10));
...
TEST_F(StateManagerTest, DemandStateTransitionWithDelayedAck)
{
  ...
  bool success = sm->demand_state_transition(task::TaskState::RUNNING, std::chrono::milliseconds(101));
